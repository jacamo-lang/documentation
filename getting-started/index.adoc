...
Regarding the organisation dimension of our application, we will simply assign roles to agents. A role is a kind of ``place holder'' for agents in groups. We will create a group that Alice and Bob will join with roles `role1` and `role2`, respectively. 
...
From agent side, the organisation provides several information for the agents consideration:

image:./screen-mind3.png[Bob's Mind]

From the agent side, we will change Alice's program so that instead of sending a message to Bob, she sends a message to an agent playing role `role2`:

[source,jasonagent,linenums]
.alice.asl
----
// new plan to  achiee say_hello
+!say_hello
...
Using this implementation, Alice's program is not tightly coupled with Bob anymore. Another agent can replace Bob as the player of `role2` and Alice keeps running correctly. 