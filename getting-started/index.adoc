...
The following commands will create a new xref:../terminology.adoc#JaCaMo_Appplication[JaCaMo application] identified by `*my1st-app*` with some initial source code for xref:../termnology.adoc#agent[agents], xref:../terminology.adoc#environment[environment] and xref:../terminology.adoc#organisation[organisation]. (The first time you run JaCaMo takes longer since it downloads all dependencies.)
...
This code states that Alice has one xref:../terminology.adoc#goal[goal], created when Alice starts running. This goal can be achieved by a xref:../terminology.adoc#plan[plan] (3rd and 4th lines) that simply sends a xref:../terminology.adoc#plan[message] to another agent named Bob telling him `greeting("hello world")`.
