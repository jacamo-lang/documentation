...
terms:: terms in beliefs are used to identify objects, which may have predicates, are relate to others, are parameter for some actions, etc. So identifiers should help the reader to recognize the object.  For instance:
...
- `open(door)`, the object door has the predicate open. It is better than `door(open)` or `opening(door)`.
- `loves(bob,alice)`, the predicate refers to a relation between two object, i.e., the  pair `bob,alice` has the predicate `loves`.
- `it_is_warm`, it is a proposition, since there are no terms in this belief.
+
...
- `!happy(bob)`, the agent intends a state of the world where bob has the predicate happy. Ideally, if the goal is achieved, then the agent beliefs in `happy(bob)`.
...
[source,jasonagent]
----
+!happy(bob) : happy(bob). // my goal was achieved already
+!happy(bob)               // a declarative goal, a state is intended
    :  lonely(bob)         // I believe that bob has predicate lonely
